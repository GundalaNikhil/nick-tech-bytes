...
**Aim:** COALESCE returns first non-NULL value. NULLIF returns NULL if two expressions are equal, otherwise first expression.
...
1. **Main Concept:** COALESCE returns first non-NULL value. NULLIF returns NULL if two expressions are equal, otherwise first expression.
